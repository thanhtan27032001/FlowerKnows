...
### US-31: Suggest a Campaign pool composition (Owner-only planning tool)

**As** Owner, **I want to** input a target bag count, bag price, expected total cost (with a tolerance), and an optional wishlist of must-include products, and have the system suggest a full pool composition, **so that** I don't have to manually pick products and quantities by trial and error to hit a budget.

**Inputs:**

| Field | Required | Description |
|---|---|---|
| `total_bags` | Yes | Target number of bags (N) |
...
| 2 | Staff wants a system-wide view, not limited to one product | Opens the "Stock Ledger" (global) screen at `/stock-ledger` (or similar route) | Shows all `stock_transaction`s across every product, **newest first**, each row showing: `product` name (since it's no longer implied by a single-product context), date/time, type (friendly label), quantity change (+/-, colored), `note`, running balance is **not** shown here (only meaningful per-product — see AC #1) |
| 2a | The Stock Ledger has more rows than fit on one page | Staff scrolls to the bottom / navigates | **Paginated** — load a fixed page size (e.g. 50 rows per page) via `LIMIT`/`OFFSET` or cursor-based pagination (`created_at` + `id` as the cursor, to stay stable even as new rows are inserted while paging), rather than fetching the entire table at once. Never query all `stock_transaction` rows unbounded — this table only grows over time |
| 2b | Staff applies a `product`, `type`, and/or date-range filter | — | Filters apply at the query level (`WHERE` clause), not client-side after fetching everything — pagination and filtering combine (e.g. "page 2 of stock_in transactions for Product X in July") |
